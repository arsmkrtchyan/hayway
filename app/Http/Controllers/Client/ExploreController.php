...
//
//        // ===== радиусы (красный = текущий шаг) =====
//        $startNear = max(1, (int)($r->get('near_km', 5)));
//        $radiusSteps = array_values(collect([$startNear, 10, 15])->unique()->all());
//
//        // ===== обе точки есть =====
//        if ($hasFrom && $hasTo) {
//            foreach ($radiusSteps as $R) {
//                $q = $this->qBothPoints($r, (float)$fromLat, (float)$fromLng, (float)$toLat, (float)$toLng, (int)$R);
//                $trips = $q->paginate(12)->through(fn(Trip $t) => $this->card($t, $R));
//                if ($trips->total() > 0) {
//                    return Inertia::render('Client/Explore', [
//                        'filters' => $this->filtersOut($r),
//                        'trips' => $trips,
//                        'amenityFilters' => $amenityFilters,
//                        'special' => ['match_radius_km' => $R],
//                        'nearOngoing' => [],
//                    ]);
//                }
//            }
...
//        // ===== базовый список =====
//        $base = $this->base($r);
//        $trips = $base->orderBy('departure_at')
//            ->paginate(12)
//            ->through(fn(Trip $t) => $this->card($t, null));
//
//        return Inertia::render('Client/Explore', [
//            'filters' => $this->filtersOut($r),
//            'trips' => $trips,
//            'amenityFilters' => $amenityFilters,
//            'special' => ['match_radius_km' => null],
//            'nearOngoing' => [],
//        ]);
//    }
//
...
//    }
//
//    private function applyCommon($q, Request $r): void
//    {
...
//        if ($r->filled('date')) {
//            try {
//                $d = \Carbon\Carbon::parse($r->string('date'));
...
//    }
//
//    /** time_mode: include|exclude|all; time_ranges: "HH:MM-HH:MM,..." (+ ночные разрывы) */
//    private function applyTime($q, Request $r): void
//    {
//        $mode = in_array($r->get('time_mode'), ['include', 'exclude', 'all'], true) ? $r->get('time_mode') : 'all';
//        $rangesStr = (string)$r->get('time_ranges', '');
//        if (!$rangesStr && $r->filled('time_from') && $r->filled('time_to')) {
//            $rangesStr = $r->string('time_from') . '-' . $r->string('time_to');
//        }
//
//        $ranges = collect(explode(',', $rangesStr))
//            ->map(function ($p) {
//                [$a, $b] = array_pad(array_map('trim', explode('-', $p, 2)), 2, '');
//                $a = preg_match('/^\d{2}:\d{2}$/', $a) ? $a . ':00' : null;
//                $b = preg_match('/^\d{2}:\d{2}$/', $b) ? $b . ':00' : null;
//                if (!$a || !$b) return null;
//                return [$a, $b];
//            })
//            ->filter()
//            ->flatMap(function ($ab) {
//                [$a, $b] = $ab;
//                return ($a <= $b) ? [[$a, $b]] : [[$a, '23:59:59'], ['00:00:00', $b]];
//            })
//            ->values();
//
//        if ($mode === 'all' || $ranges->isEmpty()) return;
//
//        if ($mode === 'include') {
//            $q->where(function ($w) use ($ranges) {
//                foreach ($ranges as [$a, $b]) {
//                    $w->orWhereRaw('(departure_at::time BETWEEN ?::time AND ?::time)', [$a, $b]);
//                }
//            });
//        } else { // exclude
//            $q->where(function ($w) use ($ranges) {
//                foreach ($ranges as [$a, $b]) {
//                    $w->whereRaw('NOT (departure_at::time BETWEEN ?::time AND ?::time)', [$a, $b]);
//                }
//            });
//        }
//    }
...
//    }
//
//    /** обе точки заданы: PAX_PAX «в городе» = обе в радиусе R; AB учитывает FREE/PAID зоны; A_PAX/PAX_B — FREE предпочтительно */
//    /** обе точки заданы */
//    private function qBothPoints(Request $r, float $fromLat, float $fromLng, float $toLat, float $toLng, int $R)
//    {
//        $q = Trip::query();
//        $this->applyCommon($q, $r);
//
...
//        WHEN type_ab_fixed AND ($dStart) <= {$R} AND ($dEnd) <= {$R}
//             AND ($startZone)=1 AND ($endZone)=1 THEN 2
//        WHEN type_ab_fixed AND ($dStart) <= {$R} AND ($dEnd) <= {$R}
//             AND ( ($startZone)=1 OR ($endZone)=1 ) THEN 3
//        WHEN type_ab_fixed AND ($dStart) <= {$R} AND ($dEnd) <= {$R} THEN 4
//        WHEN type_a_to_pax AND ($startZone)=1 THEN 5
//        WHEN type_pax_to_b AND ($endZone)=1   THEN 6
//        WHEN type_a_to_pax AND ($dStart) <= {$R} AND ($startZone)=2 THEN 7
//        WHEN type_pax_to_b AND ($dEnd)   <= {$R} AND ($endZone)=2   THEN 8
//        ELSE 99
//      END
//    ";
//
//        $walkStart = "CASE WHEN start_free_km IS NULL THEN NULL
//                       WHEN ($dStart) <= start_free_km THEN 0
//                       ELSE ($dStart - start_free_km) END";
//        $walkEnd = "CASE WHEN end_free_km IS NULL THEN NULL
//                     WHEN ($dEnd) <= end_free_km THEN 0
//                     ELSE ($dEnd - end_free_km) END";
//
//        $q->select([
//            'trips.*',
...
//            DB::raw("($walkStart) as start_walk_km_to_free"),
//            DB::raw("($walkEnd)   as end_walk_km_to_free"),
//            DB::raw("($rank)      as rank_type"),
//        ])
//            ->whereRaw("($rank) < 99")                // <-- фикс вместо HAVING
//            ->orderBy('rank_type')
//            ->orderByRaw('(COALESCE(d_start_km,0)+COALESCE(d_end_km,0)) asc')
...
//    }
//
//    /** одна точка: PAX_PAX «в городе» = точка в R; AB — точка в R; A_PAX/PAX_B — FREE предпочтительнее */
//    /** одна точка задана */
//    private function qSinglePoint(Request $r, ?float $fromLat, ?float $fromLng, ?float $toLat, ?float $toLng, int $R)
//    {
//        $q = Trip::query();
//        $this->applyCommon($q, $r);
//
...
//        WHEN type_ab_fixed  AND ($dStart) <= {$R} THEN 2
//        WHEN type_a_to_pax  AND ($startZone)=1     THEN 3
//        WHEN type_a_to_pax  AND ($dStart) <= {$R} AND ($startZone)=2 THEN 4
//        WHEN type_pax_to_b  AND ($dStart) <= {$R} THEN 5
//        ELSE 99
//      END
//    " : "
//      CASE
//        WHEN type_pax_to_pax AND ($dEnd) <= {$R} THEN 1
//        WHEN type_ab_fixed  AND ($dEnd) <= {$R} THEN 2
//        WHEN type_pax_to_b  AND ($endZone)=1     THEN 3
//        WHEN type_pax_to_b  AND ($dEnd) <= {$R} AND ($endZone)=2 THEN 4
//        WHEN type_a_to_pax  AND ($dEnd) <= {$R} THEN 5
//        ELSE 99
//      END
//    ";
//
...
//            ->whereRaw("($rank) < 99")                // <-- фикс вместо HAVING
//            ->orderBy('rank_type')
//            ->orderByRaw($hasFrom ? 'd_start_km asc' : 'd_end_km asc')
//            ->orderBy('departure_at')
//            ->orderBy('price_amd');
//
//        return $q;
//    }
//
//    private function card(Trip $t, ?int $matchRadius): array
//    {
...
//
//        $walkStart = is_numeric($t->getAttribute('start_walk_km_to_free')) ? max(0, (float)$t->getAttribute('start_walk_km_to_free')) : null;
//        $walkEnd = is_numeric($t->getAttribute('end_walk_km_to_free')) ? max(0, (float)$t->getAttribute('end_walk_km_to_free')) : null;
//
