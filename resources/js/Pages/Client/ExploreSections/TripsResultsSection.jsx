...
const AMENITY_OPTIONS = ["Կոնդիցիոներ", "USB լիցքավորում", "Wi-Fi", "Մանկական աթոռ", "Ծխելու թույլտվություն", "Կենդանիներ թույլատրվում են"];
...
export default function TripsResultsSection({ trips = [] }) {
    ...
    const visibleTrips = useMemo(() => filteredTrips.slice(0, VISIBLE_LIMIT), [filteredTrips]);
    ...
    return (
        <div className="relative flex min-h-[100svh] w-full items-center justify-center bg-gradient-to-b from-sky-50 via-slate-50 to-emerald-50 px-3 py-4 md:px-6 md:py-6">
            <motion.div key="results" initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.2, ease: "easeOut" }} className="relative flex w-full max-w-6xl flex-col gap-4 rounded-[24px] border border-slate-200 bg-white/98 p-4 shadow-md md:p-6">
                <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
                    <div>
                        <p className="text-[11px] font-semibold uppercase tracking-[0.18em] text-emerald-600">HAYWAY · TRIPS</p>
                        <h1 className="mt-1 text-xl font-semibold text-slate-900 md:text-2xl">Գտնված ուղևորություններ</h1>
                        <p className="text-xs text-slate-500">Դեմո ցուցադրում՝ հիմնված քո որոնման պարամետրերի և ակտիվ filter-ների վրա։</p>
                    </div>
                        ...
                        {!mainTrip ? (
                            ...
                            <>
                                ...
                                <AnimatePresence mode="wait">
                                    {showAmenitiesPanel ? (
                                        <motion.div key="amenities-panel" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} transition={{ duration: 0.2, ease: "easeOut" }}>
                                            <AmenitiesPanel draftAmenities={amenitiesDraft} onDraftToggle={toggleDraftAmenity} onConfirm={handleAmenitiesConfirm} onCancel={handleAmenitiesCancel} />
                                        </motion.div>
                                    ) : (
                                        otherTrips.length > 0 && (
                                                ...
                                                <div className="flex items-center justify-between">
                                                    ...
                                                    {hiddenCount > 0 && (
                                                        <p className="text-[11px] text-slate-500">
                                                            + <span className="font-semibold">{hiddenCount.toLocaleString("hy-AM")}</span> այլ առաջարկ (չեն ցուցադրվում այս դեմոյում)
                                                        </p>
                                                    )}
                                                </div>
...
function CompactFilterSidebar({ sortMode, onSortChange, timeRanges, onTimeRangeToggle, onlyHighRating, onOnlyHighRatingChange, amenitiesFilter, activeFiltersCount, onClearFilters, onOpenAmenitiesPanel }) {
    return (
        <aside className="space-y-4 rounded-2xl border border-slate-200/90 bg-white/98 p-3.5 text-xs shadow-sm lg:sticky lg:top-4">
            ...
            <div>
                ...
                <button type="button" onClick={onOpenAmenitiesPanel} className="inline-flex w-full items-center justify-between gap-2 rounded-xl border border-emerald-200 bg-emerald-50 px-3 py-1.5 text-[11px] font-medium text-emerald-800 hover:bg-emerald-100">
                    <span className="inline-flex items-center gap-1.5">
                        <Wind className="h-3.5 w-3.5" />
                        <span>Բացել հարմարությունների panel-ը</span>
                    </span>
                    {amenitiesFilter.length > 0 && <span className="inline-flex h-5 min-w-[20px] items-center justify-center rounded-full bg-emerald-600 text-[10px] font-semibold text-white">{amenitiesFilter.length}</span>}
                </button>
            </div>
        </aside>
    );
...
function AmenitiesPanel({ draftAmenities, onDraftToggle, onConfirm, onCancel }) {
    return (
        <div className="rounded-2xl border border-emerald-100 bg-emerald-50/70 p-4">
            <p className="text-[11px] font-semibold text-emerald-800">Հարմարություններ</p>
            <p className="mt-1 mb-3 text-[11px] text-emerald-700/80">Ընտրիր այն հարմարությունները, որոնք կարևոր են քեզ համար․ ընտրությունը ուժի մեջ կմտնի միայն «Հաստատել ընտրությունները» կոճակը սեղմելուց հետո։</p>
            ...
            <div className="grid gap-1.5 sm:grid-cols-2">
                {AMENITY_OPTIONS.map((label) => {
                    ...
                    return (
                        ...
                        >
                            <span className="inline-flex items-center gap-2">
                                <span className="inline-flex h-7 w-7 items-center justify-center rounded-full bg-emerald-100 text-emerald-700">{Icon && <Icon className="h-4 w-4" />}</span>
                                <span>{label}</span>
                            </span>
        ...
        <div className={`flex gap-3 ${isBig ? "flex-col md:flex-row md:items-center md:justify-between" : "flex-col"}`}>
            ...
            <div className={`mt-1 flex flex-col items-end gap-1.5 text-right ${isBig ? "md:min-w-[180px]" : ""}`}>
                ...
                <div className="mt-1 flex items-center gap-1.5">
                    <motion.button type="button" whileHover={{ y: -1, scale: 1.02 }} whileTap={{ scale: 0.97 }} className="inline-flex items-center gap-1.5 rounded-full bg-gradient-to-r from-[rgb(34,211,238)] to-[rgb(45,212,191)] px-4 py-1.5 text-xs font-semibold text-white shadow-sm">
