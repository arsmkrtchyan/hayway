...
export default function NotificationsBell({ className = "" }) {
    ...
    // сколько новых (после последнего просмотра)
    const [unread, setUnread] = useState(0);
    // сколько всего pending по активным рейсам
    const [openPending, setOpenPending] = useState(0);
    const [latest, setLatest] = useState([]);

    const timer = useRef(null);
    const prevUnread = useRef(0);

    // разрешение на браузерные Notification
    useEffect(() => {
        if ("Notification" in window && Notification.permission === "default") {
            try {
                Notification.requestPermission().catch(() => {});
    ...
    // периодический опрос backend
    useEffect(() => {
        let stopped = false;

        const tick = async () => {
            try {
                const r = await fetch("/driver/pending-requests/state", {
                    headers: { Accept: "application/json" },
                });
                if (!r.ok) throw new Error("state " + r.status);
                const d = await r.json();

                const nextUnread = Number(d?.unread_count ?? 0);
                const nextOpen   = Number(d?.open_pending ?? 0);
                const list       = Array.isArray(d?.latest) ? d.latest : [];

                setUnread(nextUnread);
                setOpenPending(nextOpen);
                setLatest(list);

                // Новых стало больше → звук + push-уведомление
    ...
    const markAsSeen = async () => {
        try {
            ...
            await fetch("/driver/pending-requests/seen", {
                method: "POST",
                headers: {
    ...
    const toggleOpen = async () => {
        const next = !open;
        setOpen(next);
        // при открытии дропдауна – помечаем как "прочитано"
        if (!open && unread > 0) {
            await markAsSeen();
        }
    };
    ...
    return (
        <div className={`relative ${className}`}>
            ...
            {open && (
                <div className="absolute right-0 mt-2 w-96 overflow-hidden rounded-xl border border-slate-200 bg-white shadow-xl">
                    <div className="border-b border-slate-200 px-3 py-2 text-sm font-semibold text-slate-900">
                        {/* здесь показываем все pending, а не только новые */}
                        Հայտեր՝ սպասման մեջ {openPending}
                    </div>
                    <div className="max-h-80 divide-y overflow-y-auto">
                        {latest.length === 0 && (
                            <div className="p-3 text-sm text-slate-500">Դատարկ է</div>
                        )}
                        {latest.map((it) => (
                            <Link
                                key={it.id}
                            ...
                            >
                                <div className="font-medium text-slate-900">
                                    {it.passenger_name || "Ուղևոր"} · {it.seats} տեղ ·{" "}
                                    {it.payment === "cash" ? "Կանխիկ" : "Քարտ"}
                                </div>
                                <div className="text-slate-600">
                                    {it.trip?.from_addr} → {it.trip?.to_addr}
                                </div>
